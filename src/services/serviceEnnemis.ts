...
export class ServiceEnnemis {
  ...
  instancierEnnemis(nbEnnemis: number) {
    ...
    for (let i = 0; i < nbEnnemis; i++) {
      point_ennemi_actuel = new Point(
        Math.random() * largeur_fenetre,
        Math.random() * hauteur_fenetre,
      );
      this.listeEnnemis.push(
        this.instancierTypeEnnemiAleatoire(point_ennemi_actuel),
      );
    }
  }
  ...
  updatePositionEnnemis() {
    this.listeEnnemis = this.listeEnnemis.map((ennemi: Ennemi) => {
      ...
      const collisionVaisseau = this.p5.serviceControleurPartie
        .getServiceVaisseau()
        .verifierCollisionAvecEnnemi(ennemi);
      this.p5.serviceControleurPartie
        .getServiceVaisseau()
        .verifierCollisionEnnemiAvecLigneVaisseau(ennemi);
  ...
  drawEnnemis() {
    ...
    this.listeEnnemis.forEach((ennemi: Ennemi) => {
      ...
      switch (typeEnnemi) {
        case "EnnemiBleu":
          const indexSpriteEnnemisBleus = this.calculerIndexSprite(
            this.indexSpritesEnnemisBleus,
            this.frequenceRafraichissementSprites,
            this.nbSpritesEnnemisBleus,
          );
          this.p5.image(
            this.images_ennemis_bleus[indexSpriteEnnemisBleus],
            ennemi.getEnnemiCercle().getPosX() -
              ennemi.getEnnemiCercle().getRayon() / 2,
            ennemi.getEnnemiCercle().getPosY() -
              ennemi.getEnnemiCercle().getRayon() / 2,
          ...
          break;
        case "EnnemiLimace":
          const indexSpriteEnnemisLimaces = this.calculerIndexSprite(
            this.indexSpritesEnnemisLimaces,
            this.frequenceRafraichissementSprites,
            this.nbSpritesEnnemisLimaces,
          );
          this.p5.image(
          ...
          break;
        case "EnnemiPieuvre":
          const indexSpriteEnnemisPieuvres = this.calculerIndexSprite(
            this.indexSpritesEnnemisPieuvres,
            this.frequenceRafraichissementSprites,
            this.nbSpritesEnnemisPieuvres,
          );
          this.p5.image(
          ...
          this.indexSpritesEnnemisPieuvres =
            this.indexSpritesEnnemisPieuvres + 1;
          break;
      }
    });
  }
      ...
      if (!ennemiDifferent) {
        if (ennemiActuel.getContientBonusBouclier()) {
          const point: Point = new Point(
            ennemiActuel.getEnnemiCercle().getPosX(),
            ennemiActuel.getEnnemiCercle().getPosY(),
          );
          this.p5.serviceControleurPartie
            .getServiceBonus()
            .faireApparaitreItemBouclier(point);
        }
        ...
        if (ennemiActuel.getContientBonusEnergie()) {
          ...
          this.p5.serviceControleurPartie
            .getServiceBonus()
            .faireApparaitreItemEnergie(point);
        }
  ...
  instancierImagesEnnemis() {
    ...
    const img_ennemi_pieuvre1 = this.p5.loadImage(
      "assets/sprites/pieuvre1.png",
    );
    const img_ennemi_pieuvre2 = this.p5.loadImage(
      "assets/sprites/pieuvre2.png",
    );
    const img_ennemi_pieuvre3 = this.p5.loadImage(
      "assets/sprites/pieuvre3.png",
    );
    const img_ennemi_pieuvre4 = this.p5.loadImage(
      "assets/sprites/pieuvre3.png",
    );
